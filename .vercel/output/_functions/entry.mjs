...
import { l as levels, g as getEventPrefix, L as Logger, A as AstroIntegrationLogger, manifest } from './manifest_h6GrAguU.mjs';
import 'node:fs';
import { e as appendForwardSlash, j as joinPaths, t as trimSlashes, s as slash, p as prependForwardSlash, r as removeTrailingForwardSlash, f as collapseDuplicateSlashes } from './chunks/astro/assets-service_trMI6DOj.mjs';
import { R as ROUTE_DATA_SYMBOL, A as AstroError, n as ResponseSentError, o as MiddlewareNoDataOrNextCalled, p as MiddlewareNotAResponse, G as GetStaticPathsRequired, q as InvalidGetStaticPathsReturn, t as InvalidGetStaticPathsEntry, u as GetStaticPathsExpectedParams, v as GetStaticPathsInvalidRouteParam, P as PageNumberParamNotFound, N as NoMatchingStaticPathFound, w as PrerenderDynamicEndpointPathCollide, x as LocalsNotAnObject, y as ASTRO_VERSION, C as ClientAddressNotAvailable, S as StaticClientAddressNotAvailable, z as renderEndpoint, B as ReservedSlotName, D as renderSlotToString, F as renderJSX, H as chunkToString, J as CantRenderPage, K as renderPage$1, O as REROUTE_DIRECTIVE_HEADER } from './chunks/astro_elZzrBSR.mjs';
...
import require$$1$1 from 'glob-parent';
import require$$0$1 from 'util';
import require$$0$2 from 'stream';
import require$$2 from '@nodelib/fs.walk';
import require$$1$2 from '@nodelib/fs.stat';
...
const scan$1 = (input, options) => {
  ...
  if (opts.unescape === true) {
    if (glob) glob = utils$b.removeBackslashes(glob);

    if (base && backslashes === true) {
      base = utils$b.removeBackslashes(base);
    }
  }
...
const expandRange = (args, options) => {
  ...
  try {
    ...
    return args.map(v => utils$a.escapeRegex(v)).join('..');
  }
...
const parse$1 = (input, options) => {
  ...
  const win32 = utils$a.isWindows(options);
  ...
  input = utils$a.removePrefix(input, state);
  len = input.length;
  ...
  if (opts.fastpaths !== false && !/(^[*!]|[/()[\]{}"])/.test(input)) {
    ...
    state.output = utils$a.wrapOutput(output, state, options);
    return state;
  }
  ...
  while (!eos()) {
    ...
    if (state.quotes === 1 && value !== '"') {
      value = utils$a.escapeRegex(value);
      prev.value += value;
      append({ value });
      continue;
    }
    ...
    if (value === ']') {
      ...
      if (opts.literalBrackets === false || utils$a.hasRegexChars(prevValue)) {
        continue;
      }

      const escaped = utils$a.escapeRegex(prev.value);
    ...
    if (value === '?') {
      ...
      if (prev && prev.type === 'paren') {
        ...
        if (next === '<' && !utils$a.supportsLookbehinds()) {
  ...
  while (state.brackets > 0) {
    ...
    state.output = utils$a.escapeLast(state.output, '[');
    decrement('brackets');
  }
...
parse$1.fastpaths = (input, options) => {
  ...
  const output = utils$a.removePrefix(input, state);
...
const picomatch$2 = (glob, options, returnState = false) => {
  ...
  const opts = options || {};
  const posix = utils$9.isWindows(options);
  const regex = isState
...
picomatch$2.test = (input, regex, options, { glob, posix } = {}) => {
  ...
  const opts = options || {};
  const format = opts.format || (posix ? utils$9.toPosixSlashes : null);
...
picomatch$2.matchBase = (input, glob, options, posix = utils$9.isWindows(options)) => {
...
const stream$2 = stream$3;
utils$h.stream = stream$2;
const string = string$1;
utils$h.string = string;
...
function generate(input, settings) {
    ...
    const staticPatterns = positivePatterns.filter((pattern) => utils$7.pattern.isStaticPattern(pattern, settings));
    const dynamicPatterns = positivePatterns.filter((pattern) => utils$7.pattern.isDynamicPattern(pattern, settings));
...
function processPatterns(input, settings) {
    ...
    if (settings.braceExpansion) {
        patterns = utils$7.pattern.expandPatternsWithBraceExpansion(patterns);
    }
    /**
     ...
     */
    return patterns.map((pattern) => utils$7.pattern.removeDuplicateSlashes(pattern));
}
/**
...
function convertPatternsToTasks(positive, negative, dynamic) {
    const tasks = [];
    const patternsOutsideCurrentDirectory = utils$7.pattern.getPatternsOutsideCurrentDirectory(positive);
    const patternsInsideCurrentDirectory = utils$7.pattern.getPatternsInsideCurrentDirectory(positive);
...
function getPositivePatterns(patterns) {
    return utils$7.pattern.getPositivePatterns(patterns);
}
tasks.getPositivePatterns = getPositivePatterns;
function getNegativePatternsAsPositive(patterns, ignore) {
    const negative = utils$7.pattern.getNegativePatterns(patterns).concat(ignore);
    const positive = negative.map(utils$7.pattern.convertToPositivePattern);
    return positive;
}
...
function groupPatternsByBaseDirectory(patterns) {
    const group = {};
    return patterns.reduce((collection, pattern) => {
        const base = utils$7.pattern.getBaseDirectory(pattern);
...
function convertPatternGroupToTask(base, positive, negative, dynamic) {
    return {
        ...
        negative,
        base,
        patterns: [].concat(positive, negative.map(utils$7.pattern.convertToNegativePattern))
    };
}
...
const fsStat$1 = require$$1$2;
const fsWalk$2 = require$$2;
...
const fsStat = require$$1$2;
const fsWalk = require$$2;
...
const _page0 = () => import('./chunks/generic_3hFFYVES.mjs');
const _page1 = () => import('./chunks/index_seUMyBCM.mjs');
const _page2 = () => import('./chunks/404_MFChYkRL.mjs');
const _page3 = () => import('./chunks/Condiciones_pI__Nos4.mjs');
const _page4 = () => import('./chunks/contacto_zOtqxg-p.mjs');
const _page5 = () => import('./chunks/Politica_giheHgBf.mjs');
const _page6 = () => import('./chunks/Sobre Mi_7GFBt2T5.mjs');
const pageMap = new Map([
    ["node_modules/.pnpm/astro@4.3.2/node_modules/astro/dist/assets/endpoint/generic.js", _page0],
    ["src/pages/index.astro", _page1],
    ["src/pages/404.astro", _page2],
    ["src/pages/Condiciones.astro", _page3],
    ["src/pages/contacto.astro", _page4],
    ["src/pages/Politica.astro", _page5],
...
const _args = {
    "middlewareSecret": "a7eda499-126c-4be6-81ab-7ee34c27fe52"
};
